...
function CusWebGLRenderer(parameters) {
    ...
    let differedTargetRenderState = null;
    ...
    this.render = function (scene, camera) {
        // // 后向渲染时，scene 需要保持为原先的目标场景
        // if (_this.differedTargetScene) {
        //     scene = _this.differedTargetScene;
        // }
        ...
        const _differdTargetScene = _this.differedTargetScene || scene;

        //
        if (scene.isScene === true) scene.onBeforeRender(_this, scene, camera, renderTarget || _currentRenderTarget);

        currentRenderState = renderStates.get(scene, renderStateStack.length);
        // currentRenderState = renderStates.get(_differdTargetScene, renderStateStack.length);
        currentRenderState.init();
        ...
        // projectObject(_differdTargetScene, camera, 0, _this.sortObjects);
        ...
        if (!_this.differedTargetScene) {
            differedTargetRenderState = { state: currentRenderState.state };
        }
    ...
    function renderObjects(renderList, scene, camera) {
        ...
        for (let i = 0, l = renderList.length; i < l; i++) {
            ...
            if (overrideMaterial) {
                let oriMat = renderItem.material;
                if (Array.isArray(oriMat)) {
                    oriMat = oriMat[0];
                }
                ...
                //         intensity: { value: 0 },
                //         color: { value: oriMat.color },
                //         map: { value: oriMat.map },
                //         ORM: {
                //             value: new Vector3(0, oriMat.roughness || .5, oriMat.metalness || 0)
                //         },
                //         aoMap: { value: oriMat.aoMap },
                //         roughnessMap: { value: oriMat.roughnessMap },
                //         metalnessMap: { value: oriMat.metalnessMap },
                //         normalMap: { value: oriMat.normalMap }
                //     });
                //     shader.defines = Object.assign({}, shader.defines, {
                //         USE_ROUGHNESSMAP: Boolean(oriMat.roughnessMap),
                //         USE_METALNESSMAP: Boolean(oriMat.metalnessMap),
                //         USE_AOMAP: Boolean(oriMat.aoMap),
                //         USE_MAP: Boolean(oriMat.map)
                //     });
                // }
    ...
    function getProgram(material, scene, object) {
        ...
        let lights, shadowsArray;
        // 使用目标场景的状态
        if (_this.differedTargetScene && differedTargetRenderState) {
            lights = differedTargetRenderState.state.lights;
            shadowsArray = differedTargetRenderState.state.shadowsArray;
        } else {
